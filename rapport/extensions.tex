...
\paragraph{Proof of Lemma 1}
Consider a configuration \e{c'} such that \e{c} $\xrightarrow{r}$ \e{c'}, where \e{r} is a channel loss transition, as described above. Then one channel \e{ch} in \e{c} of length \e{n} $leq$ \e{k}, $\xi(ch) = w_1..w_{p-1}\bullet m \bullet w_{p+1}..w_n$ loses the symbol \e{m}, resulting in the channel evaluation $\xi(ch') = w_1..w_n$ in \e{c'}. Then \e{c'} is a actually a view of \e{c}.
