...
\begin{tikzpicture} [->,auto,node distance=2.6cm,line width=0.4mm]
  ...
  \node[state] (5) [below of=2] {$s_5$};
  \node[state] (6) [below of=3] {$s_6$};
  \node[state] (7) [below of=4] {$s_7$};
  \node[state] (8) [below of=5] {$s_8$};
  \node[state] (9) [below of=6] {$s_9$};

  \path[every node/.style={font=\sffamily\small}]
    (1) edge node [above] {SND} (2)
        edge [loop above] node {A,B,C} (1)
    (2) edge node [above] {SND} (3)
        edge [loop above] node {B,C,a} (2)
        edge node [left] {A} (4)
    (3) edge node [left] {A} (5)
        edge [loop above] node {B,C,a,b} (3)
    (4) edge node [above] {SND} (5)
        edge [loop below] node {A,B,C} (4)
    (5) edge node [above] {SND} (6)
        edge [loop below] node {C,A,b} (5)
        edge node [left] {B} (7)
    (6) edge node [left] {B} (8)
        edge [loop below] node {A,C,b,c} (6)
    (7) edge node [above] {SND} (8)
        edge [loop below] node {A,B,C} (7)
    (8) edge node [above] {SND} (9)
        edge [loop below] node {A,B,c} (8)
        edge [bend left=120] node [left] {C} (1)
    (9) edge [bend right=120] node [left] {C} (2)
        edge [loop below] node {A,B,c,a} (9);
...
\begin{tikzpicture} [->,auto,node distance=2.6cm,line width=0.4mm]
  ...
  edge [loop above] node {$ch_A$!A} (1)
    (2) edge node [above] {Rcv} (3)
    (3) edge node [right] {$ch_M$?b} (4)
  edge [loop above] node {$ch_A$!B} (3)
    (4) edge node [below] {Rcv} (5)
    (5) edge node [below] {$ch_M$?b} (6)
  edge [loop below] node {$ch_A$!C} (5)
