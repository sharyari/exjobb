...
Generate the set $post(\gamma(V_k))$: For every \e{con} $\in$ $\gamma(V_k)$, \e{r} $\in$ $\delta$, compute $r(con)$.
...
\item
\textbf{Reachable configurations}


Computing the set $\mathcal{R}_k$ can be done in a multitude of ways, the simplest being to iteratively calculate the post-image of each configuration in the set. That is, $\mathcal{R}_k$ is initially $\conf{s^0, \xi^0}$, then for each $c \in \mathcal{R}_k^i$, $c \in \mathcal{R}_k^{i+1}$ and for each $r \in \delta$, $r(c) \in \mathcal{R}_k^{i+1}$ if $size(r(c)) \leq k$. 

\end{enumerate}

The pseudo-code implementing algorithm \ref{alg1} can be seen in listing \ref{naive}. 
