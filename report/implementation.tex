...
\label{part3}
\todo[inline]{I do mean the empty word below, and not the initial channel content.}
...
\subsection{Evaluation of Algorithm \ref{verificationalgorithm}}
...
\todo[inline]{Ok, this took me an embarrissingly long time to figure out. The proof relies on the fact that if a concretization is valid, it is itself a valid configuration (one could think of it as the configuration $con$ loses the empty symbol during a message loss resulting in some configuraiton $c$ with the same state and evaluation). Should I maybe add this as its own lemma?}
...
\todo[inline]{You commented that this was difficult to follow. lemma 4.3 is the original, 4.4 a reformulation of 4.3}
\begin{lemma}
...
\todo[inline]{I could express the above lemma in the way I do below, abstracting the actual views away for the proof}
\begin{lemma}
...
\todo[inline]{I updated the caption in the hopes of making it more clear.}
\begin{figure}
\centering
...
\begin{lemma}
let $n_0,n_1\ldots n_m$ be nodes, such that for each $i \leq 0$, $n_i$ has been created in the \e{i}:th iteration of the reachability analysis algorithm, reached by a node $n_{i-1}$ over an edge $e_{i-1}$. Then the shortest path from the initial node $n_0$ to node $n_m$ will necessarily be a path $e_0...e_{i-1}$. The trace $t = n_0,n_1\ldots n_m$ is a minimal trace and is of length $m$.
\end{lemma}
