...
We maintain a \e{set} $X_k = \{v | size(v) < k\}$ of views such that all views are uniquely stored in $X_k$. On line 3 of algorithm \ref{verificationalgorithm}, the concretization, post-image and abstraction functions are applied in sequence on each element $v \in X_k$, until $X_k$ reaches a \e{fix-point}, i.e. $Apost(X_k) = X_k$. A fix-point is guaranteed to be reached, as the set $V_k$ is a finite set. Assuming that an alphabet and a set of transitions \e{R} describing the transitions are known, the algorithm must compute the fix-point reached by continued application of the abstract post-image of a set (line 3), and also the set $\mathcal{R}_k$ of reachable configurations (line 2).
