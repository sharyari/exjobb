...
In the following section, we explain the semantics of a lossy channel system, which describes the behaviour of the system. The operational behaviour of \e{LCS} induces the inifinite-state transition system \e{LTS} = (\e{C}, $\rightarrow$) where $C$ is a set of configurations and $\rightarrow$ $\subseteq (C \times C)$ is the smallest transition relation defined as follows:
  \begin{itemize}
    ...
    \item For each transmission transition $( s, ch!m, s' )$ in $\delta$ and channel evaluation $\xi$ and $\xi'$, $\conf{s, \xi} \xrightarrow{ch!m} \conf{s', \xi'}$, $\xi'(ch) = \xi(ch)\bullet m$, and $\xi'(ch')=\xi(ch')$ for all $ch' \in Ch \setminus ch$. This means that the control state and the evaluation of the configuratoin change, so that the message $m$ is appended to the content of the channel $ch$.
    \item For each reception transition $( s, ch?m, s )$ in $\delta$ and channel evaluation $\xi$ and $\xi'$, $\conf{s, \xi} \xrightarrow{ch!m} \conf{s', \xi'}$ such that $\xi(ch)=m \bullet \xi'(ch) $ and $\xi'(ch')=\xi(ch')$ for all $ch' \in Ch \setminus ch$. This means that the control state and the evaluation of the configuratoin change, so that the message $m$ is removed from the content of the channel $ch$.
...
We refer to the act of taking a transition $c' \xrightarrow{r} c$, as \emph{firing} the transition $r=\conf{s,\tau,s'}$ from $c$ to $c'$ such that $state(c)=s$ and $state(c')=s'$.
...
A channel system can potentially reach a \e{bad} state, i.e. a state representing unintended behaviour. Suppose $Bad$ is a set of bad configurations of a lossy transition system \e{LTS}, as defined in \ref{LTS}. We call a sequence \e{t} = $c_0c_1\ldots c_n$ of configurations a \e{trace}, if $c_i \rightarrow c_{i+1}, 0 \leq i \leq (n-1)$ and we say that $c_n$ is reachable from $c_0$. We call a trace a \e{bad trace} if there exists $c^{bad}$ $\in Bad$ such that $c^{bad}$ is a sub-configuration of $c_n$. If \e{t} is the shortest bad trace in the system, we call it a \e{minimal bad trace}.
