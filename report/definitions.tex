...
A channel system can potentially reach a \e{bad} state, i.e. a state representing unintended behaviour. Suppose $Bad$ is a set of bad configurations of a lossy transition system \e{LTS}, as defined in \ref{LTS}. We call a sequence \e{t} = $c^0c^1\cdots c^n$ of configurations a \e{trace}, if $c^i \rightarrow c^{i+1}, 0 \leq i \leq (n-1)$ and we say that $c^n$ is reachable from $c^0$. We call a trace a \e{bad trace} if there exists $c^{bad} \in Bad$ such that $c^{bad}_S = c^n_S$ and $c^{bad}_E \subword c^{n}_E$. If \e{t} is the shortest bad trace in the system, we call it a \e{minimal bad trace}.
