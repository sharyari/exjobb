...
The abstraction function $\alpha_k: C\rightarrow 2^{C_k}$ maps a configuration $c$ into the powerset $V$ of views of size up to $k$, such that for each $v\in V$, $\{v_E \subseteq c_E\}$, $v_S = c_S$ and $|v|) \leq k$.

\paragraph{Example.} Suppose \e{c} is a configuration $\conf{s_1,r_2,ab,cd}$. The configuration is of size 2 and $\alpha_2^3(c)$ is the set
...
The concretisation function $\gamma_k: 2^{C_k} \rightarrow 2^C$ returns, for a given set of views \e{V}, the set of configurations that can be reconstructed from the views in $V$, in other words, $\gamma_k(V) = \{c \in C$ | $\alpha_k(c) \subseteq V$\}
...
As a result of lemma \ref{lemma1}, if $\gamma_k^{k+1} \cap Bad$ = $\emptyset$ for any \e{k} $\geq$ 1, then $\gamma_l^{l+1}(V) \cap Bad$ = $\emptyset$ for any $l \geq k$. \todo[inline]{You commented that this was not true. I changed this a little bit $\gamma_l^{l+1}$ instead of $\gamma_k^l$, but I doubt that was what you meant. Why would this be wrong?} Given a set of bad states \e{Bad}, a set of initial states \e{I} and a set of transitions, verification of a system can be done with algorithm \ref{alg1}, which was first presented in \cite{parosh}.
