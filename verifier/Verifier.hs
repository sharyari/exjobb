...
verify tries rules initial symbols k =
  ...
  in
  if isSafe > 0 then trace "Bad state entered, K= " $ traceShow isSafe T.empty else
    if isSafe2 > 0 then
...
verify' (trie,seen) rules symbols k c = let nextIteration = alpha (step (gamma (trie,seen) symbols k False ) rules ) k in
...
verify2 (trie,seen) rules symbols k c = let nextIteration = alpha (step (gamma (trie,seen) symbols k True ) rules ) k in
  if ((getSize (fst nextIteration)) == getSize trie) then
    fst nextIteration
  else
    verify' nextIteration rules symbols k (c-1)
