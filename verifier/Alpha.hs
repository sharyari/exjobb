...
alpha (confs, seen, list) k =
  alpha' (sortBy (comparing fst) list) k (confs,[],seen)
...
alpha' (x:xs) k (confs,new,seen) =
  let
    state = fst x
    relevant' = L.map snd $ L.takeWhile (\y -> (fst y) == (fst x)) (x:xs)
    newConfs = addViews' relevant' k S.empty
  in
    alpha' (L.dropWhile (\y -> (fst y) == (fst x)) (x:xs)) k
    (mapAddNode confs state newConfs, L.zip (takeRepeat state) (S.toList newConfs) ++ new, seen) -- This line would fail if there would be more than 1M equal-state
